...
def unpack(filename):
   ...
   #     print(plot_list[j],'\n\n\n')
    ...
    for l in range(1, len(plot_list)):
        ...
        p.addPlot(dataset(t, xl, xh, y, col=c, errs=0, errsM=errsm, errsP=errsp, norm=n))
    return p
  ...
  #      else:
    ...
    for p in rest:
        if (p_env.ratio!=1):
            ...
            ax.errorbar(pts, [z for z in p.Y], xerr = x_errors, yerr = (p.errsM, p.errsP), color=p.col, fmt = '', linewidth = .75, ls = 'none')
        else:
            ax[0].step(p.Xl, [z for z in p.Y], where = 'post', color=p.col, linewidth = .75, label=p.title, linestyle='-')
            ax[0].errorbar(pts, [z for z in p.Y], xerr = x_errors, yerr = (p.errsM, p.errsP), color=p.col, fmt = '', linewidth = .75, ls = 'none')
            ax[1].step(p.Xl, [x/(y+eps) for x, y in zip(p.Y, p_env.plots[0].Y[:len(p.Y)])], where = 'post', color=p.col, linewidth = .75, label=p.title)
        #if(p_env.errs == 1):
...
def plot_scale_bands(plots, ax1, ax2, pts, x_errors, **kwargs):
    ...
    for p in plots:
        if plots.index(p) == 0:
            ...
            if (ax2!=0):
                ax2.errorbar(pts, [(x+eps)/(y+eps) for x, y in zip(p.Y, data.Y)], xerr = x_errors, yerr = ([(x+eps)/(y+eps) for x, y in zip(p.errsM, data.Y)],
                    [(x+eps)/(y+eps) for x, y in zip(p.errsP, data.Y)]), fmt = '.', markersize='0.01', color = p.col, linewidth = .75, label = p.title)
